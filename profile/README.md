# Grading Accuracy Research Group

Welcome to the Grading Accuracy Study, a research initiative coordinated through the IRB at UC Berkeley. This organization is dedicated to studying and improving the accuracy of grading systems across educational settings.

## Affiliated Researchers

- **Andrew Bray**, Statistics, UC Berkeley
- **Joshua Grossman**, Statistics, UC Berkeley
- **Nikita Jayaprakash**, Demography, UC Berkeley
- **Mine Çetinkaya-Rundel**, Statistics, Duke University

## About Our Research

Our study examines how different grading methodologies and practices impact the accuracy and fairness of student assessment. We investigate:

- Automated vs. manual grading accuracy
- Consistency of grading across different evaluators
- Impact of grading methodologies on student outcomes
- Tools and techniques for improving grading accuracy

## Our Repositories

This organization contains repositories supporting our research, including data analysis tools, study materials, and documentation of our findings. The data, analysis, and manuscript corresponding to the primary publication is held in a private repository that will be made public after the blinded version has made it's way through scientific review.

## Resources

**Collaborative Tools:**
- Password-protected Google Drive: [Access Research Materials](https://drive.google.com/drive/folders/1z6eIQrNYc92RI49FyAcLD2D_9qgfUw-I)
- GitHub discussions for research collaboration
- Issue tracking for active research questions

## Data & Privacy

This research is conducted under UC Berkeley IRB oversight. All data handling follows strict ethical guidelines and privacy protections. Please review our data management policies before accessing or contributing to data-related repositories.

## Contact & Collaboration

For questions about the research or to discuss potential collaboration:
- Open an issue in the relevant repository
- Check individual repository contacts and maintainers
- Reach out through UC Berkeley's official channels

---

**Affiliated Institution:** University of California, Berkeley

*Last updated: 2026*
